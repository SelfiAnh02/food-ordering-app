            ...
            products.map((p, idx) => {
              ...
              return (
                <tr key={p.id ?? idx} className="border-b hover:bg-gray-50 transition">
                  <td className={`${cellPad} text-center text-gray-700 text-sm`}>{idx + 1}</td>
                  ...
                  <td className={`${cellPad} text-center font-semibold`}>{formatPrice(p.price)}</td>
                  <td className={`${cellPad} text-center`}>{categoryLabel}</td>
                  <td
                    className={`${cellPad} text-center ${
                      (p.stock ?? 0) <= 5 ? "text-red-600 font-semibold" : "text-gray-700"
                    }`}
                  >
                    {p.stock ?? 0}
                  </td>
